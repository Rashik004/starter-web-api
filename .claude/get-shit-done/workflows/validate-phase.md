...
```bash
AUDITOR_MODEL=$(node "F:/PromineoRepo/MPP/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-nyquist-auditor --raw)
NYQUIST_CFG=$(node "F:/PromineoRepo/MPP/.claude/get-shit-done/bin/gsd-tools.cjs" config get workflow.nyquist_validation --raw)
```
...
```
Task(
  prompt="Read F:/PromineoRepo/MPP/.claude/agents/gsd-nyquist-auditor.md for instructions.\n\n" +
...
**State B (create):**
1. Read template from `F:/PromineoRepo/MPP/.claude/get-shit-done/templates/VALIDATION.md`
...
node "F:/PromineoRepo/MPP/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(phase-${PHASE}): add/update validation strategy"
```
