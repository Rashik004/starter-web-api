...
- **Global scope**: `F:/PromineoRepo/MPP/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `F:/PromineoRepo/MPP/.claude/notes/`)
...
2. Glob `F:/PromineoRepo/MPP/.claude/notes/*.md` (if directory exists) — global notes
...
Global (F:/PromineoRepo/MPP/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `F:/PromineoRepo/MPP/.claude/notes/` — works in any directory
